...
.. envvar:: NVTE_ALLOW_NONDETERMINISTIC_ALGO
   ...
   :Type: ``int`` (0 or 1)
   :Default: ``1``
   :Description: :Description: Allow non-deterministic algorithms for Transformer Engine execution. When set to ``0``, only deterministic algorithms are allowed. This is relevant for both PyTorch and JAX attention implementations.
