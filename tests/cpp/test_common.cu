...
std::pair<scale_inv_meta, scale_inv_meta> get_scales(const NVTEShape& shape,
  ...
  if (scaling_mode == NVTE_MXFP8_1D_SCALING) {
    ...
    size_t scale_dim_Y_rowwise =
        DIVUP_TO_MULTIPLE(first_dim, mxfp8_scale_alignment(scale_tensor_alignment_Y_rowwise));
    size_t scale_dim_X_rowwise =
        DIVUP_TO_MULTIPLE(DIVUP(last_dim, block_size_X_rowwise),
                          mxfp8_scale_alignment(scale_tensor_alignment_X_rowwise));
    ret_rowwise.shape = {scale_dim_Y_rowwise, scale_dim_X_rowwise};

    const size_t block_size_Y_colwise = 32;
    size_t scale_dim_Y_colwise =
        DIVUP_TO_MULTIPLE(DIVUP(first_dim, block_size_Y_colwise),
                          mxfp8_scale_alignment(scale_tensor_alignment_Y_colwise));
    size_t scale_dim_X_colwise =
        DIVUP_TO_MULTIPLE(last_dim, mxfp8_scale_alignment(scale_tensor_alignment_X_colwise));
  ...
  if (scaling_mode == NVTE_BLOCK_SCALING_2D) {
    ...
    {
      auto scale_dim_0 = DIVUP(first_dim, 128lu);
#ifdef __HIP_PLATFORM_AMD__
      auto scale_dim_1 = DIVUP(last_dim, 128lu);
#else
      auto scale_dim_1 = DIVUP(DIVUP(last_dim, 128lu), 4) * 4;
#endif
      ret_rowwise.shape = {scale_dim_0, scale_dim_1};
    }
    {
      auto scale_dim_0 = DIVUP(last_dim, 128lu);
#ifdef __HIP_PLATFORM_AMD__
      auto scale_dim_1 = DIVUP(first_dim, 128lu);
#else
      auto scale_dim_1 = DIVUP(DIVUP(first_dim, 128lu), 4) * 4;
#endif
...
Tensor::Tensor(const std::string& name,
  ...
  if (rowwise) {
    ...
    data_rowwise_.emplace(product(shape), rowwise_type);
    tensor_.set_rowwise_data(data_rowwise_->gpu_buffer(), rowwise_type, shape);
  }
  ...
  if (columnwise) {
    ...
    data_columnwise_.emplace(product(columnwise_shape), colwise_type);
    tensor_.set_columnwise_data(data_columnwise_->gpu_buffer(), colwise_type, columnwise_shape);
  }
...
std::array<size_t, 4> get_scale_tensor_dims(const size_t rows,
    ...
    if (scaling_mode == NVTE_MXFP4_1D_SCALING) {
      ...
      alignment_Y = alignment_X = mxfp8_scale_alignment(1);
    } else {
      alignment_Y = 1;
      alignment_X = 1;
    }
