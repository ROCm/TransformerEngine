...
Tensor::Tensor(const NVTEShape &shape, const DType type, const NVTEScalingMode &scaling_mode, const bool _is_tensor_2x) : is_tensor_2x(_is_tensor_2x) {
  ...
  if (isFp8Type(type)) {
    ...
    (void)cudaMalloc((void**)&scale_inv, scale_size);  // NOLINT(*)
    (void)cudaMemset(scale_inv, 0, scale_size);
    scale_inv_cpu_data_ = std::make_unique<unsigned char[]>(scale_size);
    std::fill_n(scale_inv_cpu_data_.get(), scale_size, 0);

    if (is_tensor_2x){
      (void)cudaMalloc((void**)&columnwise_dptr, total_size);  // NOLINT(*)
      (void)cudaMemset(columnwise_dptr, 0, total_size);
...
void Tensor::to_cpu() const {
  ...
  if (isFp8Type(dtype())) {
    if (is_tensor_scaling(tensor_.scaling_mode())) {
      (void)cudaMemcpy(amax_cpu_data_.get(), tensor_.amax(), sizeof(float),
                 cudaMemcpyDeviceToHost);
      (void)cudaMemcpy(scale_cpu_data_.get(), tensor_.scale(), sizeof(float),
                 cudaMemcpyDeviceToHost);
    }
    (void)cudaMemcpy(scale_inv_cpu_data_.get(), tensor_.scale_inv(),
                     product(tensor_.scale_inv_shape()) * typeToSize(tensor_.scale_inv_dtype()),
                     cudaMemcpyDeviceToHost);
    if (is_tensor_2x) {
      (void)cudaMemcpy(columnwise_cpu_data_.get(), tensor_.columnwise_dptr(),
                       product(tensor_.columnwise_shape()) * typeToSize(tensor_.dtype()),
                       cudaMemcpyDeviceToHost);
      (void)cudaMemcpy(
          columnwise_scale_inv_cpu_data_.get(), tensor_.columnwise_scale_inv(),
...
void Tensor::from_cpu() const {
  ...
  if (isFp8Type(dtype())) {
    ...
    (void)cudaMemcpy(tensor_.scale_inv(), scale_inv_cpu_data_.get(),
                     product(tensor_.scale_inv_shape()) * typeToSize(tensor_.scale_inv_dtype()),
                     cudaMemcpyHostToDevice);
    if (is_tensor_2x) {
      (void)cudaMemcpy(tensor_.columnwise_dptr(), columnwise_cpu_data_.get(),
                       product(tensor_.columnwise_shape()) * typeToSize(tensor_.dtype()),
                       cudaMemcpyHostToDevice);
      (void)cudaMemcpy(
          tensor_.columnwise_scale_inv(), columnwise_scale_inv_cpu_data_.get(),
          product(tensor_.columnwise_scale_inv_shape()) * typeToSize(tensor_.scale_inv_dtype()),
          cudaMemcpyHostToDevice);
    }
  }
