 ...
 ************************************************************************/
//
// Test host for the fused MFMA forward kernel (attn_fwd_mfma.h).
// Uses the same test infrastructure and CPU reference as test_fwd.cpp.
//
// Build: cmake -B build && cmake --build build && ./build/test_fwd_mfma
...
// ---------------------------------------------------------------------------
// Main forward correctness + performance test (MFMA variant)
// ---------------------------------------------------------------------------

template <typename DataType, typename Config>
void test_run_attn_fwd_mfma_kernel(
    int bs, float dropout_p, int warmup_iters, int test_iters, bool check_correctness, bool dump_err)
{
    using Launcher = AttnForwardMfmaKernelLauncher<DataType, Config>;
    ...
    std::cout << "\n===== run_attn_fwd_mfma_kernel Test =====" << std::endl;
...
// ---------------------------------------------------------------------------
// Corner-case test: explicit Q seqlens provided by caller (MFMA variant)
// ---------------------------------------------------------------------------

template <typename DataType, typename Config>
void test_run_attn_fwd_mfma_with_seqlens(const std::vector<int>& h_cu_seqlens_q,
...
struct RunFwdMfma {
    ...
    void operator()(int bs, float dropout_p, int warmup, int iters, bool check, bool dump) const {
        test_run_attn_fwd_mfma_kernel<DataType, Config>(bs, dropout_p, warmup, iters, check, dump);
    }
};
...
{
    std::cout << "\n========== MFMA Fwd Correctness (bf16, SEQ_KV 2..16, bs=30720) ==========" << std::endl;

    TestRunner<2, 16>::run<float, 30720, 32, 128, 256, false, CausalMaskType::DISABLE>(
        RunFwdMfma{}, 0.0f, 1, 1, true, true);

    std::cout << "\n========== MFMA Fwd Correctness (bf16, mixed Q=0/1, SEQ_KV=8, bs=128) ==========" << std::endl;
    {
        using MixedConfig = FmhaKernelConfig<8, 8, 128, 256, false, CausalMaskType::DISABLE>;
        test_run_attn_fwd_mfma_kernel<float, MixedConfig>(128, 0, 1, 1, true, true);
    }

    std::cout << "\n========== MFMA Fwd Performance (bfloat16, SEQ_KV 2..16) ==========" << std::endl;
    TestRunner<2, 16>::run<hip_bfloat16, 30720, 32, 128, 256, false, CausalMaskType::DISABLE>(
        RunFwdMfma{}, 0.0f, 3, 5, false, false);

    std::cout << "\n========== MFMA Corner: Empty segments (even batches active, bs=128) ==========" << std::endl;
    {
        ...
        test_run_attn_fwd_mfma_with_seqlens<float, CornerConfig>(
            h_cu_seqlens_q, h_cu_seqlens_q_padded, h_padded_q_to_batch,
            total_padded_q, 0.0f, true, true, "Empty segments");
    }

    std::cout << "\n========== MFMA Corner: Q padded > actual (2 slots per batch, bs=128) ==========" << std::endl;
    {
        const int corner_bs = 128;
        std::vector<int> h_cu_seqlens_q(corner_bs + 1);
        std::vector<int> h_cu_seqlens_q_padded(corner_bs + 1);
