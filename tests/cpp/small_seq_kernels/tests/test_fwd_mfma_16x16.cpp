...
//
// Build: cmake -B build && cmake --build build && ./build/test_fwd_mfma_16x16
...
// ---------------------------------------------------------------------------
// Main forward correctness + performance test (MFMA 16x16x16 variant)
// ---------------------------------------------------------------------------
    ...
    int bs, float dropout_p, int warmup_iters, int test_iters, bool check_correctness, bool dump_err)
{
    ...
    std::cout << "\n===== run_attn_fwd_mfma_16x16_kernel Test =====" << std::endl;
    ...
    std::cout << "  softmax_lse buffer:  " << lse_bytes / 1e6 << " MB" << std::endl;
    std::cout << std::endl;
    ...
    HIP_CHECK(hipFree(d_dropout_mask)); HIP_CHECK(hipFree(d_O)); HIP_CHECK(hipFree(d_softmax_lse));
...
// ---------------------------------------------------------------------------
// Corner-case test: explicit Q seqlens provided by caller (MFMA 16x16 variant)
// ---------------------------------------------------------------------------

template <typename DataType, typename Config>
void test_run_attn_fwd_mfma_16x16_with_seqlens(const std::vector<int>& h_cu_seqlens_q,
...
{
    std::cout << "\n========== MFMA 16x16 Fwd Correctness (bf16, SEQ_KV 2..16, bs=30720) ==========" << std::endl;

    TestRunner<2, 16>::run<float, 30720, 32, 128, 256, false, CausalMaskType::DISABLE>(
        RunFwdMfma16x16{}, 0.0f, 1, 1, true, true);
    ...
    TestRunner<2, 16>::run<hip_bfloat16, 30720, 32, 128, 256, false, CausalMaskType::DISABLE>(
        RunFwdMfma16x16{}, 0.0f, 3, 5, false, false);

    std::cout << "\n========== MFMA 16x16 Corner: Empty segments (even batches active, bs=128) ==========" << std::endl;
    {
    ...
    std::cout << "\n========== MFMA 16x16 Corner: Q padded > actual (2 slots per batch, bs=128) ==========" << std::endl;
    {
        const int corner_bs = 128;
        std::vector<int> h_cu_seqlens_q(corner_bs + 1);
        std::vector<int> h_cu_seqlens_q_padded(corner_bs + 1);
    ...
    // --- Tiling boundary tests ---
    // Helper lambda to build uniform Q seqlens
    auto build_uniform_q = [](int bs, int sq,
                              std::vector<int>& csq, std::vector<int>& csqp,
                              std::vector<int>& q2b) {
        csq.resize(bs + 1);
        csqp.resize(bs + 1);
        csq[0] = csqp[0] = 0;
    ...
    // Test 1: sq=17, skv=17 — both Q and KV cross tile boundary (2x2 = 4 tiles)
    std::cout << "\n========== Tiled Fwd: sq=17, skv=17 (2x2 tiles, bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
    ...
    // Test 2: sq=17, skv=16 — only Q crosses tile boundary (2x1 tiles)
    std::cout << "\n========== Tiled Fwd: sq=17, skv=16 (Q tiled, KV single, bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
        std::vector<int> csq, csqp, q2b;
        int tot = build_uniform_q(bs, 17, csq, csqp, q2b);
    ...
    // Test 3: sq=16, skv=17 — only KV crosses tile boundary (1x2 tiles)
    std::cout << "\n========== Tiled Fwd: sq=16, skv=17 (Q single, KV tiled, bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
        std::vector<int> csq, csqp, q2b;
    ...
    // Test 4: sq=1, skv=17 — single Q row, KV tiled
    std::cout << "\n========== Tiled Fwd: sq=1, skv=17 (single Q, KV tiled, bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
        std::vector<int> csq, csqp, q2b;
    ...
    // Test 5: mixed Q lengths (0..17), skv=17 — variable Q with tiling
    std::cout << "\n========== Tiled Fwd: mixed Q=0..17, skv=17 (bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
        std::mt19937 gen2(99);
        std::vector<int> csq, csqp, q2b;
        csq.resize(bs + 1); csqp.resize(bs + 1);
        csq[0] = csqp[0] = 0;
        std::uniform_int_distribution<int> qdist(0, 17);
        for(int b = 0; b < bs; b++) {
            int ql = qdist(gen2);
            csq[b + 1]  = csq[b]  + ql;
            csqp[b + 1] = csqp[b] + ql;
    ...
    // Test 6: sq=skv=16 through tiled path — verify single-tile still correct
    std::cout << "\n========== Tiled Fwd: sq=16, skv=16 (single tile, sanity, bs=2048) ==========" << std::endl;
    {
        const int bs = 2048;
        std::vector<int> csq, csqp, q2b;
        int tot = build_uniform_q(bs, 16, csq, csqp, q2b);
