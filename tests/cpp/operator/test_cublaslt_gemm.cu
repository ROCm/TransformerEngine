...
// Grouped GEMM tests.
//
// GEMMTestSuite above exercises the single-GEMM path (nvte_cublas_gemm), which
// never touches the grouped-backend dtype gate. These tests instead drive
// nvte_multi_tensor_gemm with NVTE_USE_CK_GROUPED_GEMM=1 so the CK grouped
// backend (ck_tile_grouped_gemm) is exercised.
//
