...
void performTest_x1(const size_t rows,
    ...
    adjust_ref_for_e8m0_scale_error("scales", mismatches_scales_indices, gpu_scales_ptr,
                                    ref_output_scales.get(), scales_stride, rows, output_cols,
                                    rowwise, ref_output.get(), otype);
    mismatches_scales = 0;
#endif
...
void performTest_x2(const size_t rows,
    ...
    adjust_ref_for_e8m0_scale_error("scales_rowwise", mismatches_scales_indices_rowwise,
                                    output.rowwise_cpu_scale_inv_ptr<fp8e8m0>(),
                                    ref_scales_rowwise.get(), scales_stride_rowwise, rows,
                                    output_cols, true, ref_output_rowwise.get(), otype);
    mismatches_scales_rowwise = 0;
    adjust_ref_for_e8m0_scale_error("scales_colwise", mismatches_scales_indices_colwise,
                                    output.columnwise_cpu_scale_inv_ptr<fp8e8m0>(),
                                    ref_scales_colwise.get(), scales_stride_colwise, rows,
                                    output_cols, false, ref_output_colwise.get(), otype);
