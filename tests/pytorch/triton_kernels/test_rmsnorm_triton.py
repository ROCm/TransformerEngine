...
def test_rmsnorm_fwd_bwd_triton(
    M, N, in_dtype_test, out_dtype_test, zero_centered_gamma
):
    ...
    if out_dtype_test[1:] == "fp8e4m3":
        ...
        scale_inv_triton = None
    # run the fwd triton path
    ln_out_triton, rsigma_triton = te_rmsnorm_fwd_fp8_noalloc_triton(
        input_tensor,
        gamma_tensor,
        epsilon,
        ln_out_triton,
        ...
        scale=scale,
        amax=amax_triton,
        scale_inv=scale_inv_triton,
    )

    ln_out_hipified, rsigma_hipified = tex.rmsnorm_fwd_fp8_noalloc(
        input_tensor,
        gamma_tensor,
        epsilon,
        scale,
    ...
    atol, _ = get_tolerances(torch.float32)
    rtol = 5e-5
    ...
    # run triton bwd
    dx_triton, dgamma_triton = te_rmsnorm_bwd_triton(
        dz_tensor,
        input_tensor,
        rsigma_triton,
        gamma_tensor,
    ...
    dx_hipified, dgamma_hipified = tex.rmsnorm_bwd(
        dz_tensor,
        input_tensor,
        rsigma_hipified,
        gamma_tensor,
...
# TODO: add fp8/bf8 once fp8 triton kernels are available
@pytest.mark.parametrize("zero_centered_gamma", all_boolean)
def test_rmsnorm_fwd_noalloc_triton(
    M, N, in_dtype_test, out_dtype_test, zero_centered_gamma
):
    in_dtype = str_to_torch_dtype(in_dtype_test)
    out_dtype = str_to_torch_dtype(out_dtype_test)
...
def test_rmsnorm_fwd_inf_triton(M, N, in_dtype, zero_centered_gamma):
    ...
    # run the triton path
    ln_out_triton = te_rmsnorm_fwd_inf_triton(
        input_tensor, gamma_tensor, epsilon, inf_ln_sm_margin, zero_centered_gamma
    )

    # run the reference hipified kernel path
    ln_out_hipified = tex.rmsnorm_fwd_inf(
        input_tensor, gamma_tensor, epsilon, inf_ln_sm_margin, zero_centered_gamma
    )
