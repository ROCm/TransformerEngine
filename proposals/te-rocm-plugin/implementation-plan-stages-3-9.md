...
### S7.4 · JAX milestone — **FIRST HALF EVIDENCED 2026-09-01**: tests/jax under the overlay - test_functions.py + test_custom_call_compute.py = 1384 passed / 172 skipped, tree verified as the overlay. Single-GPU sweep DONE 2026-09-01: 2519 passed / 6663 skipped / 0 genuine failures under the overlay in 58 min (the 14 'errors' are multi-process launcher test files mis-collected by bare pytest - fixture-not-found at setup, they need their shell wrappers; run them via multi_process_launch.sh in CI). Distributed suites probed 2026-09-02: softmax 192 passed + layernorm/helper 109 passed clean (CI-shaped: 4 GPUs, RCCL_MSCCL_ENABLE=0); dense and layernorm_mlp HANG bare-locally - positional/stateful class (dense hung at the same 9-test point even with the previously-hanging param deselected; both spun 8h with frozen logs) - report to the jax-suite owners; these run in CI runners, not bare on this box. REMAINING: the multi-process launcher suites (CUDA-shaped launcher) and the named MaxText-class workload with its signed band (workload choice is the certifier's)
