...
* TE_HIPBLASLT_ALGO_SAVE - filename to save algorithm selection data to, can be the same as a filiname to load

It is not guaranteed that algorim selection data file created with one version of TE or hipBlasLt will work with other verisons. Even if it works, it is highly recommended to perform algoritm selection tuning again when switch to new libraries versions becaue new hipBLASLt may have new optimized algorithms.
