...
Blockwise FP8 GEMM support on ROCm (gfx94x and gfx95x)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Blockwise FP8 GEMM is supported on gfx94x and gfx95x GPUs through the HipKittens backend and is enabled by default.
...
On gfx95x there are two blockwise FP8 GEMM kernels, selected at runtime by the NVTE_BLOCKWISE_FP8_POWER_OF_2_SCALE environment variable:
