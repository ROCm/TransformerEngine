...
def train(args, model, device, train_loader, optimizer, epoch, use_amp, use_fp8):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if use_amp:
            ...
            with te.fp8_model_init(enabled=use_fp8):
                output = model(data)
...
def main():
    ...
    # try to use the TE fused adam optimizer with fp32 master_weights
    options = {
        "lr": 5e-4,
        "betas": (0.9, 0.999),
        "eps": 1e-08,
    ...
    optimizer = te.optimizers.FusedAdam(model.parameters(), master_weights=True, **options)
