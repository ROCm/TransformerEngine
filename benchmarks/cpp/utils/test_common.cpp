...
size_t create_seed_from_tensor_name(const std::string& tensor_name) {
  auto full_name = "benchmark/" + tensor_name;
...
Tensor::Tensor(const std::string& name,
  ...
  if (total_size != 0) {
    if (rowwise) {
      (void)hipMalloc((void**)&dptr_rowwise, total_size);  // NOLINT(*)
      (void)hipMemset(dptr_rowwise, 0, total_size);
      cpu_data_rowwise_ = std::make_unique<unsigned char[]>(total_size);
      std::fill_n(cpu_data_rowwise_.get(), total_size, 0);
    }
    if (columnwise) {
      (void)hipMalloc((void**)&dptr_columnwise, total_size);  // NOLINT(*)
      (void)hipMemset(dptr_columnwise, 0, total_size);
  ...
  if (isFp8Type(type) || isFp4Type(type)) {
    if (scaling_mode == NVTE_DELAYED_TENSOR_SCALING) {
      (void)hipMalloc((void**)&amax, sizeof(float));  // NOLINT(*)
      (void)hipMemset(amax, 0, sizeof(float));
      (void)hipMalloc((void**)&scale, sizeof(float));  // NOLINT(*)
      (void)hipMemset(scale, 0, sizeof(float));
      ...
      (void)hipMalloc((void**)&rowwise_scale_inv, sizeof(float));  // NOLINT(*)
      if (rowwise) {
      ...
      if (rowwise) {
        (void)hipMalloc((void**)&rowwise_scale_inv, rowwise_scale_size);  // NOLINT(*)
        (void)hipMemset(rowwise_scale_inv, 0, rowwise_scale_size);
      ...
      if (columnwise) {
        (void)hipMalloc((void**)&columnwise_scale_inv, columnwise_scale_size);  // NOLINT(*)
        (void)hipMemset(columnwise_scale_inv, 0, columnwise_scale_size);
...
void Tensor::to_cpu() const {
  ...
  if (rowwise_) {
    (void)hipMemcpy(cpu_data_rowwise_.get(),
  ...
  if (columnwise_) {
    ...
    (void)hipMemcpy(cpu_data_columnwise_.get(),
  ...
  if (isFp8Type(dtype()) || isFp4Type(dtype())) {
    if ((tensor_.scaling_mode() == NVTE_DELAYED_TENSOR_SCALING)) {
      if (tensor_.amax() != nullptr){
        (void)hipMemcpy(amax_cpu_data_.get(),
                  tensor_.amax(),
                  sizeof(float),
                  hipMemcpyDeviceToHost);
      }
      (void)hipMemcpy(scale_cpu_data_.get(),
                 tensor_.scale(),
                 sizeof(float),
                 hipMemcpyDeviceToHost);
    }
    ...
    if (rowwise_) {
      ...
      (void)hipMemcpy(rowwise_scale_inv_cpu_data_.get(),
    ...
    if (columnwise_) {
      ...
      (void)hipMemcpy(columnwise_scale_inv_cpu_data_.get(),
...
void Tensor::from_cpu() const {
  ...
  const size_t size = bytes(s, tensor_.dtype());
  if (rowwise_) {
    (void)hipMemcpy(tensor_.get_rowwise_data().data_ptr, cpu_data_rowwise_.get(), size,
               hipMemcpyHostToDevice);
  }
  if (columnwise_) {
    (void)hipMemcpy(tensor_.get_columnwise_data().data_ptr, cpu_data_columnwise_.get(), size,
               hipMemcpyHostToDevice);
  }
  if (isFp8Type(dtype()) || isFp4Type(dtype())) {
    if ((tensor_.scaling_mode() == NVTE_DELAYED_TENSOR_SCALING)
      ...
      if (tensor_.amax() != nullptr){
        (void)hipMemcpy(tensor_.amax(), amax_cpu_data_.get(), sizeof(float), hipMemcpyHostToDevice);
      }
      (void)hipMemcpy(tensor_.scale(), scale_cpu_data_.get(), sizeof(float), hipMemcpyHostToDevice);
    }
    auto [rowwise_scale_meta, colwise_scale_meta] = get_scales(s, tensor_.scaling_mode());
    if (rowwise_) {
      auto scale_size = rowwise_scale_meta.bytes();
      (void)hipMemcpy(tensor_.get_rowwise_scale_inv().data_ptr,
                 rowwise_scale_inv_cpu_data_.get(), scale_size,
                 hipMemcpyHostToDevice);
    }
    if (columnwise_) {
      auto scale_size = colwise_scale_meta.bytes();
      (void)hipMemcpy(tensor_.get_columnwise_scale_inv().data_ptr,
...
#endif // #ifndef BENCHMARK_STATIC_DEFINE
...
static void fillUniformLinearBufferDevice(T* dst_dev,
  ...
  float* tmp = nullptr;
  NVTE_CHECK_CUDA(hipMalloc(&tmp, N * sizeof(float)));

  float* tmp_sign = nullptr;
  if (random_sign) {
    NVTE_CHECK_CUDA(hipMalloc(&tmp_sign, N * sizeof(float)));
  }

  hiprandGenerator_t gen;
  NVTE_CHECK(hiprandCreateGenerator(&gen, HIPRAND_RNG_PSEUDO_PHILOX4_32_10) == HIPRAND_STATUS_SUCCESS);
  NVTE_CHECK(hiprandSetPseudoRandomGeneratorSeed(gen, seed) == HIPRAND_STATUS_SUCCESS);
  NVTE_CHECK(hiprandGenerateUniform(gen, tmp, N) == HIPRAND_STATUS_SUCCESS);

  if (random_sign) {
    NVTE_CHECK(hiprandGenerateUniform(gen, tmp_sign, N) == HIPRAND_STATUS_SUCCESS);
  }
  ...
  if (random_sign)
   hipLaunchKernelGGL(( affine_transform_cast_signs<T, true>), dim3(grid), dim3(block), 0, 0, 
      tmp, tmp_sign, dst_dev, N, lo, hi);
  else
   hipLaunchKernelGGL(( affine_transform_cast_signs<T, false>), dim3(grid), dim3(block), 0, 0, 
      tmp, nullptr, dst_dev, N, lo, hi);

  NVTE_CHECK_CUDA(hipGetLastError());

  if (dst_cpu != nullptr) {
    NVTE_CHECK_CUDA(hipMemcpy(dst_cpu, dst_dev, N * sizeof(T), hipMemcpyDeviceToHost));
  }

  NVTE_CHECK(hiprandDestroyGenerator(gen) == HIPRAND_STATUS_SUCCESS);
  NVTE_CHECK_CUDA(hipFree(tmp));
  if (tmp_sign)
    NVTE_CHECK_CUDA(hipFree(tmp_sign));
}
...
void fillCase_special(Tensor *t) {
  ...
  if constexpr (Case == InputsFillCase::zeros) {
    TRANSFORMER_ENGINE_TYPE_SWITCH_FP16_FP32_ONLY(t->dtype(), InputType, {
      ...
      NVTE_CHECK_CUDA(hipMemset(dst_dev, 0, size * sizeof(InputType)));
