...
#ifndef USE_ROCM
#define FP4_TYPE_SUPPORTED (TORCH_HIP_VERSION >= 12080)
...
class Tensor {
 public:
  ...
  ~Tensor() {
    ...
    if (data_ptr != nullptr) {
      (void)hipFree(data_ptr);
    }
    if (scale_inv != nullptr) {
      (void)hipFree(scale_inv);
    }
    if (columnwise_data_ptr != nullptr){
      (void)hipFree(columnwise_data_ptr);
    }
    if (columnwise_scale_inv != nullptr){
      (void)hipFree(columnwise_scale_inv);
    }
  }
