...
- **AITER source**: the tree QoLA/TE actually builds is cloned on-demand to `build/cmake/qola/third_party/aiter/` at the commit pinned in `transformer_engine/common/qola_manifest.toml`. `transformer_engine/common/CMakeLists.txt` runs one `qola_checkout_aiter()` for the whole build (before `add_subdirectory(ck_fused_attn)`, since CK-JIT needs the sources at configure time); each consumer then builds its own `[[modules]]` subset from that shared tree via `qola_build_modules(GROUP ...)` / `qola build --group <name>` (`ck_fused_attn`, `aiter_gemm`). The in-tree `3rdparty/*/aiter` checkouts are stale — read the built tree (and check its HEAD) when reasoning about the linked ABI.
