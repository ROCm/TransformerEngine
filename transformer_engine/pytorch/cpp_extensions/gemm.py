...
__all__ = [
    ...
    "general_grouped_gemm_single_input",
]
...
def general_grouped_gemm_single_input(
    A: torch.Tensor,
    B: List[torch.Tensor],
    out: List[torch.Tensor],
    out_dtype: torch.dtype,
    ...
    """
    Optimized TN layout Grouped GEMM that takes a single contiguous input tensor
    and avoids torch.split overhead.
    """
    num_gemms = len(B)
    ...
    bias = tex.te_general_grouped_gemm_single_input(
        A,
        transa,
        B,
        transb,
