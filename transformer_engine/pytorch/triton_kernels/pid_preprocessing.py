...
# The Triton PID-remapping helpers (remap_xcd_chunked/remap_xcd/pid_grid) are copied
# from the aiter project (https://github.com/ROCm/aiter),
# commit 8dfaf0a, aiter/ops/triton/utils/_triton/pid_preprocessing.py.
# get_num_xcds is copied from aiter/ops/triton/utils/device_info.py.
# Vendored here so the route-list MoE kernels have no runtime dependency on aiter.
...
def get_num_xcds():
    # Currently, you can't query this programmatically. For gfx942/gfx950 it's 8, so we
    # hardcode that here.
    return 8
