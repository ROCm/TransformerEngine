    ...
    def __torch_dispatch__(cls, func, types, args, kwargs=None):
        ...
        if func == aten.split.Tensor:
            ...
            if IS_HIP_EXTENSION:
                if get_device_compute_capability() == (12, 5):
                    # gfx1250 MX pre-swizzle layout requires both dims padded to multiple of 4
                    row_pad_multiple, col_pad_multiple = 4, 4
                else:
                    # ROCm/HIP backend uses an unpadded scale-inv layout (see
                    # `MXFP8Quantizer.make_empty`), so applying the padding here would produce a
                    # per-shard scale-inv whose dim-0 does not match the destination scale-inv
                    # allocated for the FSDP2 local shard.
                    row_pad_multiple, col_pad_multiple = 1, 1
            else:
                # Padding requirements: rowwise dim0 should be divisible by 128, columnwise dim0
                # should be divisible by 4
                row_pad_multiple, col_pad_multiple = 128, 4
