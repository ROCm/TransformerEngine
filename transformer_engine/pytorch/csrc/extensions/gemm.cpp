...
// Optimized version that avoids torch.split overhead by slicing in C++
std::optional<std::vector<at::Tensor>> te_general_grouped_gemm_single_input(
    at::Tensor A_full, bool transa, std::vector<py::handle> B, bool transb,
    std::optional<std::vector<at::Tensor>> D, DType D_type, std::vector<int64_t> m_splits,
    std::vector<at::Tensor> bias, DType bias_type, bool single_output,
    std::vector<at::Tensor> pre_gelu_out, bool grad, std::vector<at::Tensor> workspace,
    size_t workspaceSize, bool accumulate, bool use_split_accumulator, int math_sm_count) {
  ...
  // Slice the input tensor in C++ instead of Python using narrow()
  // This avoids torch.split overhead in Python layer
  int64_t offset = 0;
  for (size_t i = 0; i < m_splits.size(); i++) {
    // Use narrow() to create a view slice without copying
    at::Tensor A_slice = A_full.narrow(0, offset, m_splits[i]);
    auto te_A = makeTransformerEngineTensor(A_slice);
    offset += m_splits[i];
    ...
    te_workspace_vector.emplace_back(
        makeTransformerEngineTensor(workspace[i].data_ptr(), std::vector<size_t>{workspaceSize},
                                    DType::kByte)
            .data());
  }
