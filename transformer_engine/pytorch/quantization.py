...
def _compute_fp8_block_scaling_support() -> Tuple[bool, str]:
    ...
    if IS_HIP_EXTENSION:
        ...
        # backend, whose kernels are compiled into libtransformer_engine only when
        # USE_HIPKITTENS_GEMM is set. CMake auto-disables it for archs other than
        # gfx942/gfx950 or when C++20 is unavailable; without it the GEMM path
        # throws at runtime, so report the recipe unavailable and let callers skip.
...
def get_align_size_for_quantization(recipe: Recipe) -> int:
    ...
    if recipe.mxfp8():
        ...
        # HK is used by default when NVTE_USE_CUTLASS_GROUPED_GEMM=1,
        # unless NVTE_USE_CK_GROUPED_GEMM=1 explicitly selects CK.
        use_hk = os.environ.get("NVTE_USE_HIPKITTENS_GROUPED_GEMM", "0") == "1"
...
class FP8GlobalStateManager:
    ...
    def autocast_exit(cls, enabled: bool, _graph: bool) -> None:
        ...
        if not cls.SKIP_FP8_REDUCTION_FOR_FSDP2 and enabled and qstate.autocast_depth == 0 and not _graph and torch.is_grad_enabled():
