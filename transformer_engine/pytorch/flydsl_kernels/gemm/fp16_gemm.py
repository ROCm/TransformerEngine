...
"""FlyDSL FP16 4-wave GEMM kernel for Transformer Engine.
...
consumes A and B as FP16 tensors shaped [M, K] and [N, K], and writes FP16,
BF16, or FP32 C shaped [M, N]. The public ``fp16_matmul`` entry point accepts
Transformer
...
from .fp16_gemm_utils import (
    ...
    make_bf16_byte_buffer_tensor as make_fp16_byte_buffer_tensor,
    pack_i32x4_i32x8,
    swizzle_128,
)
...
def make_fp16_inputs(M, N, K, device="cuda"):
    """Generate FP16 A[M,K] and B[N,K] inputs."""
    A = (torch.randn(M, K, device=device) * 0.5).to(torch.float16)
    B = (torch.randn(N, K, device=device) * 0.5).to(torch.float16)
    return A, B
...
def _compile_kernel(
    ...
    @fx.struct
    class SharedStorage:
        # Each logical 256x64 FP16 page is two independent 128x64 half-pages.
...
def fp16_matmul(
    ...
    """TE-facing TN FP16 GEMM adapter.
    ...
    if a.ndim != 2 or b.ndim != 2:
        raise ValueError(
            f"FlyDSL FP16 TN expects rank-2 operands, got A{tuple(a.shape)} "
    ...
    if a.dtype != torch.float16 or b.dtype != torch.float16:
        raise TypeError(
            "FlyDSL FP16 GEMM expects both operands to have torch.float16 dtype, "
...
def doGemm(
    ...
    """Launch the private K-specialized FP16 core.
    ...
    if M_runtime % _BLOCK_M != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL FP16 GEMM requires M to be a multiple of {_BLOCK_M}, "
            f"got M={M_runtime}"
        )

    if N_runtime % _BLOCK_N != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL FP16 GEMM requires N to be a multiple of {_BLOCK_N}, "
            f"got N={N_runtime}"
        )

    if K_runtime % _BLOCK_K != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL FP16 GEMM requires K to be a multiple of {_BLOCK_K}, "
            f"got K={K_runtime}"
        )
    ...
    if num_k_tiles < 4:
        raise FlyDSLUnsupportedError(
            f"FlyDSL FP16 GEMM requires at least 4 K{_BLOCK_K} tiles, "
