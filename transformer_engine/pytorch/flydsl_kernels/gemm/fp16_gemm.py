...
consumes A and B as FP16 tensors shaped [M, K] and [N, K], and writes FP16 C
shaped [M, N]. The public ``fp16_matmul`` entry point accepts Transformer
...
def _compile_kernel(K: int, use_xcd_remap: bool = True):
    ...
    def kernel_gemm(
        ...
        c_tile_base_bytes = c_tile_base_elems * fx.Index(2)  # C is FP16.
...
def fp16_matmul(
    ...
    if c.dtype != torch.float16:
        raise TypeError(
            f"The current FlyDSL FP16 kernel stores torch.float16 output, got {c.dtype}"
        )
