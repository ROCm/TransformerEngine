...
"""FlyDSL MXFP8 NN 4-wave GEMM implementation.

This specialization preserves the validated MXFP8 TN compute, scale, MFMA,
accumulator, and epilogue pipelines. A is physically row-major [M, K].
B is physically row-major [N, K], staged as XOR-swizzled [K128, N128] LDS,
and reconstructed with the validated four-read ds_read_b64_tr_b8 path.

Raw scales enter as A rowwise [M, K/32] and B columnwise [K/32, N].
Orientation-aware prepacking converts both to the common iteration-major
...
def _compile_kernel(
    ...
    def kernel_gemm(
        ...
        # A remains ordinary row-major [M, K].
        ...
        # B is the selected MXFP8 columnwise payload, physically [K, N].
        # Load the K-major source directly into the XOR-swizzled physical LDS
        # image [K128, N128] consumed by ds_read_b64_tr_b8.
        ...
        def load_b_subtile_ni_regs(lds_b, scale_tile, sn, ni):
            ...
            b_ni = load_b_frag_transpose(lds_b[sn], local_n_tile)
...
def do_gemm(
    ...
    A and B are shaped [M, K] and [K, N]. As/Bs are preshuffled packed
    ...
    if M_runtime % _BLOCK_M != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NN GEMM requires M to be a multiple of {_BLOCK_M}, "
            f"got M={M_runtime}"
        )
    if N_runtime % _BLOCK_N != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NN GEMM requires N to be a multiple of {_BLOCK_N}, "
            f"got N={N_runtime}"
        )
    if K_runtime % _BLOCK_K != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NN GEMM requires K to be a multiple of {_BLOCK_K}, "
            f"got K={K_runtime}"
        )
    num_k_tiles = K_runtime // _BLOCK_K
    if num_k_tiles < 4:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NN GEMM requires at least 4 K{_BLOCK_K} tiles, "
...
def mxfp8_matmul(
    ...
    """Launch MXFP8 NN GEMM with one transpose-read operand.

    Contract:
        a:       [M, K] row-major FP8 payload
        a_scale: [M, K/32] raw rowwise E8M0 scales
        b:       [K, N] row-major columnwise-quantized FP8 payload
        b_scale: [K/32, N] raw columnwise E8M0 scales
        D:       [M, N] float16, bfloat16, or float32 output

    The B payload remains physically [K, N]. The kernel stages that K-major
    source into the XOR-swizzled LDS image and uses ds_read_b64_tr_b8 to
    reconstruct the MFMA B fragment. Scale
    prepacking resolves the source orientation before launch, so both packed
    scale tensors use the common [K/128, dim] kernel representation.
    """
    if a.ndim != 2 or b.ndim != 2:
        raise ValueError(
            f"FlyDSL MXFP8 NN expects rank-2 operands, got "
    ...
    if kb != k:
        raise ValueError(
            f"Incompatible MXFP8 NN operands: "
            f"A{tuple(a.shape)} and B{tuple(b.shape)}"
        )
    ...
    if a.dtype not in supported_fp8_dtypes or b.dtype not in supported_fp8_dtypes:
        raise TypeError(
            "FlyDSL MXFP8 NN expects E4M3 or E5M2 payloads independently, "
    ...
    _debug(
        f"NN kernel inputs: a={tuple(a.shape)}, b={tuple(b.shape)}, "
