...
"""FlyDSL MXFP8 NT 4-wave GEMM implementation.

This specialization preserves the validated MXFP8 TN compute, scale, MFMA,
accumulator, and epilogue pipelines while applying the validated
ds_read_b64_tr_b8 path to both operands. A is physically [K, M] and B is
physically [K, N]. Each source tile is staged as XOR-swizzled [K128, X128] LDS.

Both raw scale tensors are columnwise, [K/32, M] and [K/32, N].
Orientation-aware prepacking converts them to the common iteration-major
[K/128, dim] uint32 representation consumed by the kernel."""
...
def _compile_kernel(
    ...
    def kernel_gemm(
        ...
        # address arithmetic below is expressed in MLIR index type.
        ...
        #   B [K, N]
        #
        # Read each K-by-X source tile in XOR-swizzled coordinate order and
        # write it linearly to LDS. swizzle_128 is self-inverse, producing the
        # physical [K128, X128] image consumed by ds_read_b64_tr_b8.
        ...
        def hot_loop_scheduler_q0_refill_a1_2n():
            # A-bottom and the B slices are transpose reads in NT.
            for _ in range_constexpr(8):
                rocdl.sched_vmem(1)
                rocdl.sched_dsrd(2)
                rocdl.sched_mfma(2)
        ...
        def hot_loop_scheduler_q_prefetch_4n():
            # Each prefetched A/B fragment uses two DS_READ_TR instructions.
            for _ in range_constexpr(8):
                rocdl.sched_dsrd(4)
                rocdl.sched_mfma(4)
        ...
        def load_transposed_frag_half(lds_page, local_x_tile, half):
            """Load one K64 portion of a fixed-X MFMA fragment.

            This is the inverse mapping validated against the working ordinary
            LDS fragment:

              source_k = lane_div_16*16 + lane_in_16//2
              source_x = local_x_tile + (lane_in_16&1)*8

            ``base ^ 0x440`` advances logical K by 8 under swizzle_128.
            The 0x2000 DS immediate advances logical K by 64.
            """
        ...
        def load_b_subtile_ni_regs(lds_b, scale_tile, sn, ni):
            ...
            b_ni = load_transposed_frag(lds_b[sn], local_n_tile)
...
def do_gemm(
    ...
    """Launch MXFP8 NT core from K-major A [K,M] and B [K,N]."""
    ...
    if M_runtime % _BLOCK_M != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NT GEMM requires M to be a multiple of {_BLOCK_M}, "
            f"got M={M_runtime}"
        )
    if N_runtime % _BLOCK_N != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NT GEMM requires N to be a multiple of {_BLOCK_N}, "
            f"got N={N_runtime}"
        )
    if K_runtime % _BLOCK_K != 0:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NT GEMM requires K to be a multiple of {_BLOCK_K}, "
            f"got K={K_runtime}"
        )
    num_k_tiles = K_runtime // _BLOCK_K
    if num_k_tiles < 4:
        raise FlyDSLUnsupportedError(
            f"FlyDSL MXFP8 NT GEMM requires at least 4 K{_BLOCK_K} tiles, "
...
def mxfp8_matmul(
    ...
    """Launch MXFP8 NT GEMM with transpose-read A and B operands.

    Contract:
        a:       [K, M] row-major FP8 payload
        a_scale: [K/32, M] raw columnwise E8M0 scales
        b:       [K, N] row-major FP8 payload
        b_scale: [K/32, N] raw columnwise E8M0 scales
        D:       [M, N] float16, bfloat16, or float32 output

    Both operands remain K-major. Each is staged as an XOR-swizzled
    [K128, X128] LDS image and reconstructed with ds_read_b64_tr_b8.
    """
    if a.ndim != 2 or b.ndim != 2:
        raise ValueError(
            f"FlyDSL MXFP8 NT expects rank-2 operands, got "
    ...
    if kb != k:
        raise ValueError(
            f"Incompatible MXFP8 NT operands: "
            f"A{tuple(a.shape)} and B{tuple(b.shape)}"
        )
    ...
    if a.dtype not in supported_fp8_dtypes or b.dtype not in supported_fp8_dtypes:
        raise TypeError(
            "FlyDSL MXFP8 NT expects E4M3 or E5M2 payloads independently, "
    ...
    if tuple(a_scale.shape) != expected_a_scale:
        raise ValueError(
            f"a_scale shape {tuple(a_scale.shape)} != expected {expected_a_scale}"
        )
    if tuple(b_scale.shape) != expected_b_scale:
        raise ValueError(
            f"b_scale shape {tuple(b_scale.shape)} != expected {expected_b_scale}"
        )
    ...
    _debug(
        f"NT kernel inputs: a={tuple(a.shape)}, b={tuple(b.shape)}, "
