...
# Rank 0 print helper with debug flag check
...
class _LayerNormLinear(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_mxfp4_enabled:
            ...
            input_quantizer_mxfp4 = MXFP4Quantizer(rowwise=True, columnwise=backward_needs_input, shuffle_B_matrix_for_aiter=False)
        ...
        # For MXFP4, we need ln_out in high precision to quantize to both MXFP4 and FP8
        if is_mxfp4_enabled:
            with_quantized_norm = False
        ...
        ln_out_mxfp4 = None  # Will be set by fused kernel if MXFP4 enabled
        ...
        if with_input_all_gather and not ub_overlap_ag_fprop:
            ...
            if fp8 and not is_mxfp4_enabled:
                ...
                if is_mxfp4_enabled and not isinstance(ln_out, QuantizedTensor) and ln_out_mxfp4_local is None:
            ...
            if is_mxfp4_enabled and fp8 and not isinstance(ln_out, QuantizedTensor):
                # For MXFP4: gather the FP4 tensor directly, use for both fwd GEMM and wgrad
            ...
            if ub_overlap_ag_fprop:
                ...
                if fp8:
                    if not isinstance(ln_out, QuantizedTensor):
                        # Quantize from bf16 to fp4 for fwd gemm and wgrad
                        # Skip if fused kernel already provided ln_out_mxfp4
                        if is_mxfp4_enabled:
                            if ln_out_mxfp4 is None:
        ...
        if not fp8:
            ...
            # Check if weight needs MXFP4 conversion
            ...
            if need_mxfp4_conversion:
                # Weight is FP8 but we need MXFP4 - dequantize and requantize
                
                high_prec_weight = weight.dequantize()
                quantized_weight = True
                
                # Directly use the quantizer to convert to MXFP4
                # MXFP4 quantizer doesn't support get_weight_workspace
                if weight_quantizer is not None:
                    weightmat = weight_quantizer.quantize(high_prec_weight)
                else:
                    weightmat = high_prec_weight
            elif not isinstance(weight, QuantizedTensor):
                quantized_weight = True

                # Configure quantizer
                if weight_quantizer is not None and not is_mxfp4_enabled:
                ...
                # Weight is already in correct format
                pass
        ...
        if is_grad_enabled:
            ...
            if inp.requires_grad and keep_fp8_weight_transpose_cache:
                if isinstance(weightmat, QuantizedTensor) and not isinstance(weightmat, MXFP4TensorBase):
            ...
            tensors_to_save, tensor_objects = prepare_for_saving(
                inputmat,
                weightmat,
                None if need_mxfp4_conversion else weight,  # Skip for MXFP4 SFT
                bias,
                ln_weight,
    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_LayerNormLinear_backward"):
            ...
            if ctx.fp8 and not ctx.keep_fp8_weight_transpose_cache:
                # Skip transpose cache creation for MXFP4 tensors
                if not isinstance(weight, MXFP4TensorBase):
                    _rank0_print(f"[LayerNormLinear BWD] Creating FP8 transpose cache\tweight_type={type(weight).__name__}")
                    create_fp8_weight_transpose_cache(weight)
                else:
                    _rank0_print(f"[LayerNormLinear BWD] Skipping FP8 cache for MXFP4\tweight_type={type(weight).__name__}")

            # ------------------------------------------------------
            # dGrad GEMM
            # ------------------------------------------------------
            # dgrad GEMM - unified path for both FP8 and FP4
            ...
            if ctx.fp8 and not ctx.keep_fp8_weight_transpose_cache:
                # Skip transpose cache clearing for MXFP4 tensors
                if not isinstance(weight, MXFP4TensorBase):
            ...
            dbeta = None
            nvtx_range_push(f"{nvtx_label}.norm")
            # with Timer("LayerNormLinear", "norm_bwd"):
...
class LayerNormLinear(TransformerEngineBaseModule):
    ...
    def _get_quantizers(self, fp8_output):
        ...
        if fp4:
            # Input: used as A in fprop, B in wgrad - can't pre-shuffle
            input_quantizer = MXFP4Quantizer(rowwise=True, columnwise=False, shuffle_B_matrix_for_aiter=False)
        else:
        ...
        if torch.is_grad_enabled():
            if fp4:
                grad_output_quantizer = MXFP4Quantizer(rowwise=True, columnwise=False)  # No shuffle for grad
