    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_LayerNormLinear_backward"):
            ...
            if is_mxfp4_enabled and ctx.fp8:
                ...
                grad_output = ctx.grad_output_quantizer_mxfp4(grad_outputs[0])
                grad_bias = None
            ...
            if ctx.requires_wgrad:
                ...
                # Deallocate input tensor if not returning layernorm output
...
class LayerNormLinear(TransformerEngineBaseModule):
    ...
    def forward(
        ...
        with self.prepare_forward(
            ...
            (
                ...
                self._get_quantizers(fp8_output, is_mxfp4_enabled)
                if not debug
                else self._get_debug_quantizers(fp8_output, is_mxfp4_enabled)
            )
            if debug:
                if not any_feature_enabled(quantizers if 'quantizers' in locals() else []):
                    # If no feature is used, then run faster implementation with debug = False.
                    quantizers = self._get_quantizers(fp8_output, is_mxfp4_enabled)
                    debug = False
    ...
    def _get_quantizers(self, fp8_output, is_mxfp4_enabled=False):
        if not self.fp8:
            return [None] * 6
    ...
    def _get_debug_quantizers(self, fp8_output, is_mxfp4_enabled: bool):
        original_quantizers = self._get_quantizers(fp8_output, is_mxfp4_enabled)
        assert TEDebugState.debug_enabled
        from ...debug.pytorch.debug_quantization import DebugQuantizer

        names = ["activation", "weight", "output", "grad_output", "grad_output_mxfp4", "grad_input"]
        return tuple(
