...
class _LayerNormLinear(torch.autograd.Function):
    ...
    def forward(
        ...
        if use_fp4 and fp8:
            ...
            # Use the FP4 GEMM handler for forward pass
            out = fp4_gemm(
                pass_type='fwd',
                weight=weightmat,
            ...
            # UB overlap not supported in FP4 path
            rs_out = None

            nvtx_range_pop(f"{nvtx_label}.gemm")
            #######################################################################
            # END FP4 FORWARD GEMM REPLACEMENT
            #######################################################################
        else:
            #######################################################################
            # ORIGINAL FORWARD GEMM (FP4=false or fp8=false)
            #######################################################################
            nvtx_range_push(f"{nvtx_label}.gemm")
    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_LayerNormLinear_backward"):
            ...
            if use_fp4 and ctx.fp8:
                #######################################################################
                # FP4 DGRAD REPLACEMENT FOR FP8 PATH
                #######################################################################
                nvtx_range_push(f"{nvtx_label}.dgrad_gemm")

                # Use the FP4 GEMM handler for dgrad
                dgrad = fp4_gemm(
                    pass_type='dgrad',
                    weight=weight,
                ...
                # Handle rs_out for overlap cases
                if rs_out is not None and ctx.ub_overlap_rs_dgrad:
                    # Need to get the high-precision result before quantization
                    if isinstance(dgrad, QuantizedTensor):
                        dgrad_result = dgrad.dequantize()
                    else:
                        dgrad_result = dgrad
                    rs_out.copy_(dgrad_result[:rs_out.shape[0], :rs_out.shape[1]])

                nvtx_range_pop(f"{nvtx_label}.dgrad_gemm")
                #######################################################################
                # END FP4 DGRAD REPLACEMENT
                #######################################################################
            else:
                #######################################################################
                # ORIGINAL DGRAD (FP4=false or fp8=false)
                #######################################################################
                nvtx_range_push(f"{nvtx_label}.dgrad_gemm")
            ...
            if ctx.requires_wgrad:
                ...
                if use_fp4 and ctx.fp8:
                    #######################################################################
                    # FP4 WGRAD REPLACEMENT FOR FP8 PATH
                    #######################################################################
                    # wgrad GEMM
                    # Note: Fuse with bgrad computation if needed
                    nvtx_range_push(f"{nvtx_label}.wgrad_gemm")

                    # Use the FP4 GEMM handler for wgrad
                    wgrad = fp4_gemm(
                        pass_type='wgrad',
                        input_tensor=ln_out_total,
                        grad_output=grad_output,
                    ...
                    # Set grad_bias_ to None (bias grad is handled separately in original code)
                    grad_bias_ = None

                    nvtx_range_pop(f"{nvtx_label}.wgrad_gemm")
                    #######################################################################
                    # END FP4 WGRAD REPLACEMENT
                    #######################################################################
                else:
                    #######################################################################
                    # ORIGINAL WGRAD (FP4=false or fp8=false)
                    #######################################################################
                    # wgrad GEMM
                    # Note: Fuse with bgrad computation if needed
                    nvtx_range_push(f"{nvtx_label}.wgrad_gemm")
                ...
                # Common post-processing for both FP4 and original wgrad paths
                if ctx.ub_bulk_wgrad:
                ...
                # Handle grad_bias_ (only defined in original wgrad path)
                if 'grad_bias_' in locals() and grad_bias is None:
                    grad_bias = grad_bias_
                if 'grad_bias_' in locals():
                    del grad_bias_
...
class LayerNormLinear(TransformerEngineBaseModule):
    ...
    def set_meta_tensor(self, fwd: bool, recipe: Recipe) -> None:
        ...
        super().set_meta_tensor(fwd, recipe )  # Pass FP4 flag  # modify-fp4   
