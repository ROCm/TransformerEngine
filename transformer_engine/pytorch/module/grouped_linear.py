...
class GroupedLinear(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        if IS_HIP_EXTENSION and use_grouped_tensor:
            # ROCm has no cuBLASLt grouped-tensor GEMM path; use the split-quantize path.
            warnings.warn(
                "use_grouped_tensor=True is not supported on ROCm; falling back to the "
                "split-quantize grouped GEMM path.",
                stacklevel=2,
            )
