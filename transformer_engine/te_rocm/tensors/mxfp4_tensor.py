...
from transformer_engine.pytorch.constants import MXFP8_BLOCK_SCALING_SIZE  # MXFP4 uses same block size
from transformer_engine.pytorch.utils import devices_match, round_up_to_nearest_multiple
...
class MXFP4Tensor(MXFP4TensorStorage, QuantizedTensor):
    ...
    def device(self):
        ...
        raise RuntimeError("MXFP4Tensor has no data!")
...
# Pickle-path pinning (plugin plan S5.2 / F15): checkpoints reference classes by
# __module__.__qualname__. Pinning the OLD path keeps both checkpoint directions working -
# pre-move checkpoints load here via the shim, post-move checkpoints load on pre-move builds
# (rollback). The shim module at the old path re-exports these same objects.
for _cls in (MXFP4Tensor, MXFP4Quantizer):
    _cls.__module__ = "transformer_engine.pytorch.tensor.mxfp4_tensor"
