...
__device__ inline T warp_reduce_on_shmem(T *data_ptr, int data_size, int lane_id) {
  ...
  if constexpr (type == ReduceFuncType::SUM) {
    ...
    val += __shfl_xor(val, 4, kThreadsPerWarp);
    val += __shfl_xor(val, 2, kThreadsPerWarp);
    val += __shfl_xor(val, 1, kThreadsPerWarp);
  } else {
    auto shfl_max = [](CompType a, CompType b) { return a > b ? a : b; };
    val = shfl_max(val, __shfl_xor(val, 16, kThreadsPerWarp));
    val = shfl_max(val, __shfl_xor(val, 8, kThreadsPerWarp));
    val = shfl_max(val, __shfl_xor(val, 4, kThreadsPerWarp));
    val = shfl_max(val, __shfl_xor(val, 2, kThreadsPerWarp));
    val = shfl_max(val, __shfl_xor(val, 1, kThreadsPerWarp));
  }
#else
  if constexpr (type == ReduceFuncType::SUM) {
