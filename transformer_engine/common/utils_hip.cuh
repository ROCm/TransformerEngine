...
inline __device__ T warp_shuffle_xor(const T & x, uint32_t idx) {  // inline __device__ T warp_shuffle_xor(const T & x, uint32_t idx) {
    return __shfl_xor(x, idx); // __shfl_xor_sync(uint32_t(-1), x, idx);
}
...
inline __device__ T warp_shuffle_down(const T & x, uint32_t idx) {
    return __shfl_down(x, idx); // __shfl_down_sync(uint32_t(-1), x, idx);
}
...
struct Converter<float2, hip_bfloat16x2>{
    static inline __device__ hip_bfloat16x2 convert(const float2 &x) {
        ...
        tmp.elt[0] = hip_bfloat16(x.x); // __float2bfloat16_rn(x.x);
        tmp.elt[1] = hip_bfloat16(x.y); // __float2bfloat16_rn(x.y);
        return tmp.raw;
#endif
    }
};
...
#pragma unroll
    for (int delta = num_elems/2; delta > 0; delta /= 2) {
        const float other_m = __shfl_down(tmp, delta); // __shfl_down_sync(0xFFFFFFFF, tmp, delta);
