...
void ck_fused_attn_fwd(size_t batch, size_t num_attn_heads, size_t num_gqa_groups,
    ...
    ck_fused_attn_fwd_impl(batch, num_attn_heads, num_gqa_groups, max_seqlen_q, max_seqlen_kv,
                           ...
                           devPtrDropoutSeed, devPtrDropoutOffset, attn_bias_type, attn_mask_type,
                           devPtrQ, devPtrK, devPtrV, devPtrBias, devPtrS, devPtrO,
                           devPtrCuSeqlensQ, devPtrCuSeqlensKV, get_datatype_str(QKV_type),
                           &workspace->data.dptr, &workspace_size, stream);
...
void ck_fused_attn_bwd(size_t batch, size_t num_attn_heads, size_t num_gqa_groups,
    ...
    */
    ...
    static bool bwd_v3         = getenv<int>("NVTE_CK_BWD_V3") == 1;
    static bool v3_atomic_fp32 = getenv<int>("NVTE_CK_V3_ATOMIC_FP32") == 1;
    static bool v3_spec        = getenv<int>("NVTE_CK_V3_SPEC") == 1;
    static int  v3_bf16_cvt    = getenv<int>("NVTE_CK_V3_BF16_CVT");

    if (v3_bf16_cvt < 0 || v3_bf16_cvt > 2) {
        NVTE_ERROR("Invalid NVTE_CK_V3_BF16_CVT value");
    }

    ck_fused_attn_bwd_impl(
        batch, num_attn_heads, num_gqa_groups, max_seqlen_q, max_seqlen_kv, head_dim, bias_b,
        bias_h, attn_scale, p_dropout, devPtrDropoutSeed, devPtrDropoutOffset, attn_bias_type, attn_mask_type,
        ...
        v3_atomic_fp32, v3_spec, v3_bf16_cvt, stream);
