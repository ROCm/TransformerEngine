...
void nvte_fused_attn_fwd(const NVTETensor Q, const NVTETensor K, const NVTETensor V,
  ...
  // o_format and qkv_scale_inv_format are accepted for parity with the upstream C API.
  // The ROCm CK/AOTriton backends derive the O layout from qkv_layout and do not support
  // FP8/MXFP8 attention (rejected in is_{ck,aotriton}_backend_supported), so these are unused here.
  (void)o_format;
  (void)qkv_scale_inv_format;
  using namespace transformer_engine;
...
void nvte_fused_attn_bwd(const NVTETensor Q, const NVTETensor K, const NVTETensor V,
  ...
  // o_format, do_format, dqkv_layout, qkv_scale_inv_format and do_scale_inv_format are accepted
  // for parity with the upstream C API. The ROCm CK/AOTriton backends derive O/dQKV layouts from
  // qkv_layout and do not support FP8/MXFP8 attention, so these are unused here.
  (void)o_format;
  (void)do_format;
  (void)dqkv_layout;
  (void)qkv_scale_inv_format;
  (void)do_scale_inv_format;
  using namespace transformer_engine;
  const Tensor *input_cu_seqlens_q = convertNVTETensorCheck(cu_seqlens_q);
  const Tensor *input_cu_seqlens_kv = convertNVTETensorCheck(cu_seqlens_kv);
  const Tensor *input_cu_seqlens_q_padded = convertNVTETensorCheck(cu_seqlens_q_padded);
