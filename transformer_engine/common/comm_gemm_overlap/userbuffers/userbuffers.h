...
void allgather2_userbuff_inplace(const int handler, const int offset, const int elements,
                                 communicator *comm, cudaStream_t stream = 0, cudaEvent_t comm_launch_event = 0);
/*
each Rank input is
...
void reducescatter2_userbuff_fp8(void *output, float *scale, const int handler, const int offset,
                                 const int elements, communicator *comm, cudaStream_t stream = 0, cudaEvent_t comm_launch_event = 0);
template <typename fp8type>
