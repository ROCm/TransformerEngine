...
static std::unique_ptr<RunnerInterface> make_fp16_runner_typed(
    DType d_dtype,
...
static std::unique_ptr<RunnerInterface> make_fp16_runner(
    DType a_dtype,
    DType b_dtype,
    DType d_dtype,
    const GroupedGemmRunContext& ctx) {
...
bool ck_tile_grouped_gemm_fp16_dispatch(DType a_dtype,
  ...
  auto runner = make_fp16_runner(a_dtype, b_dtype, d_dtype, ctx);
  if (!runner) {
    return false;
  }
