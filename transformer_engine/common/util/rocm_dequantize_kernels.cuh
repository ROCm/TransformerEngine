...
constexpr size_t ROCM_CHUNK_DIM_Y = 128;
constexpr size_t ROCM_CHUNK_DIM_X = 128;
constexpr size_t ROCM_THREADS_PER_CHUNK = 128;
constexpr size_t ROCM_BUFFERS_NUM = 2;

constexpr size_t ROCM_ELEMS_PER_THREAD = 16;
constexpr size_t ROCM_BUFFER_DIM_Y = 16;
constexpr size_t ROCM_BUFFER_DIM_X = ROCM_CHUNK_DIM_X;  // 128
constexpr size_t ROCM_SHMEM_DIM_Y = ROCM_BUFFER_DIM_Y;  // 16
constexpr size_t ROCM_SHMEM_DIM_X = ROCM_BUFFER_DIM_X;  // 128

constexpr size_t ROCM_THREADS_PER_CHUNK_X_ROWWISE =
    ROCM_CHUNK_DIM_X / ROCM_ELEMS_PER_THREAD;  //   8 = 128 / 16
constexpr size_t ROCM_THREADS_PER_CHUNK_X_COLWISE = ROCM_CHUNK_DIM_X;  // 128
constexpr size_t ROCM_ITERATIONS = ROCM_CHUNK_DIM_Y / ROCM_BUFFER_DIM_Y;  //   8 = 128 / 16
...
__global__ void __launch_bounds__(ROCM_THREADS_PER_CHUNK)
  ...
  constexpr size_t SCALES_ROWWISE_PER_CHUNK_Y = ROCM_CHUNK_DIM_Y;                //  128
  constexpr size_t SCALES_ROWWISE_PER_CHUNK_X = ROCM_CHUNK_DIM_X / SCALE_DIM_X;  //    4 = 128 / 32

  constexpr size_t SCALES_COLWISE_PER_CHUNK_Y = ROCM_CHUNK_DIM_Y / SCALE_DIM_Y;  //    4 = 128 / 32
  constexpr size_t SCALES_COLWISE_PER_CHUNK_X = ROCM_CHUNK_DIM_X;                //  128
  ...
  alignas(128) __shared__ IType in_sh[ROCM_SHMEM_DIM_Y][ROCM_SHMEM_DIM_X];
  alignas(128) __shared__ OType out_sh[ROCM_SHMEM_DIM_Y][ROCM_SHMEM_DIM_X];

  for (int iter = 0; iter < ROCM_ITERATIONS; iter++) {
    const int chunk_it_offset_y = chunk_offset_Y + iter * ROCM_BUFFER_DIM_Y;
    const int chunk_it_offset_x = chunk_offset_X;

    copy_2d_to_shared<IType, VECTOR_WIDTH, IS_ALIGNED>(&in_sh[0][0], input_ptr, chunk_it_offset_x, 
                      chunk_it_offset_y, cols, ROCM_SHMEM_DIM_Y,
                      ROCM_SHMEM_DIM_X, rows, cols);
    __syncthreads();

    const int scale_offset_Y =
        USE_ROWWISE_SCALING ? (scales_rowwise_chunk_offset_Y + iter * ROCM_BUFFER_DIM_Y + tid_rowwise_Y)
                            : (scales_colwise_chunk_offset_Y + (iter * ROCM_BUFFER_DIM_Y) / SCALE_DIM_Y);
    ...
    if constexpr (USE_ROWWISE_SCALING) {
      ...
      Vec<OType, ROCM_ELEMS_PER_THREAD> out;
...
#pragma unroll
      for (int i = 0; i < ROCM_BUFFER_DIM_Y; i++) {
...
static void rocm_mxfp8_dequantize(const Tensor &input, Tensor *output, cudaStream_t stream) {
  ...
  const dim3 block(ROCM_THREADS_PER_CHUNK);
