...
inline rsmi_status_t call(const char *symbol, ArgTs... args) {
  using FuncT = rsmi_status_t(ArgTs...);
 ...
 */
inline const char *get_nvml_error_string(rsmi_status_t rc) {
  using FuncT = const char *(rsmi_status_t);
...
#define NVTE_CHECK_CUDA_NVML(expr)                                                             \
  do {                                                                                         \
    const rsmi_status_t status_NVTE_CHECK_CUDA_NVML = (expr);                                   \
    if (status_NVTE_CHECK_CUDA_NVML != RSMI_STATUS_SUCCESS) {                                         \
