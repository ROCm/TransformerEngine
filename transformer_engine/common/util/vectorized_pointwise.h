...
template <int nvec, bool aligned, typename ComputeType, typename Param,
          ComputeType (*OP)(ComputeType, const Param &), typename InputType, typename OutputType>
__launch_bounds__(unary_kernel_threads) __global__
    void cast_dbias_kernel(const InputType *input, OutputType *output,
                           ComputeType *dbias_workspace, const ComputeType *scale,
                           ComputeType *amax, ComputeType *scale_inv, Param p, const size_t N,
                           const size_t num_aligned_elements) {
  VectorizedLoader<InputType, nvec, aligned> loader(input, N);
  VectorizedStorer<OutputType, nvec, aligned> storer(output, N);
  VectorizedStorer<ComputeType, nvec, aligned> dbias_storer(dbias_workspace, N);
...
#pragma unroll
    for (int i = 0; i < nvec; ++i) {
      const ComputeType val = static_cast<ComputeType>(loader.separate()[i]);
      // For cast+dbias, OP is just identity, but we keep it for potential future fusions.
      ComputeType temp = OP(val, p);

      // Write pre-scaled value to workspace for reduction
      dbias_storer.separate()[i] = temp;
...
template <int nvec, bool aligned, typename ComputeType, typename Param,
          ...
          typename OutputType>
__launch_bounds__(unary_kernel_threads) __global__
    void cast_dbias_dgelu_kernel(const InputType *grad, const InputType *input, OutputType *output,
                                 ComputeType *dbias_workspace, const ComputeType *scale,
                                 ComputeType *amax, ComputeType *scale_inv, Param p, const size_t N,
                                 const size_t num_aligned_elements) {
  VectorizedLoader<InputType, nvec, aligned> grad_loader(grad, N);
  VectorizedLoader<InputType, nvec, aligned> input_loader(input, N);
...
#pragma unroll
    for (int i = 0; i < nvec; ++i) {
      ...
      const ComputeType input_val = static_cast<ComputeType>(input_loader.separate()[i]);

      // The core dgelu fusion logic
      const ComputeType elt = grad_val * OP(input_val, p);

      // Write pre-scaled value to workspace for reduction
      dbias_storer.separate()[i] = elt;

      if constexpr (is_fp8<OutputType>::value) {
        __builtin_assume(max >= 0);
        max = fmaxf(fabsf(elt), max);
      }

      const ComputeType final_elt = elt * s;
      output_storer.separate()[i] = static_cast<OutputType>(final_elt);
    }
...
void VectorizedCastAndDBiasKernelLauncher(const InputType *input, OutputType *output,
                                          ...
                                          cudaStream_t stream) {
  if (N != 0) {
    auto align = CheckAlignment(N, nvec, input, output, dbias_workspace);
    ...
    switch (align) {
      case Alignment::SAME_ALIGNED:
        cast_dbias_kernel<nvec, true, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            input, output, dbias_workspace, scale, amax, scale_inv, params, N,
            num_aligned_elements);
        break;
      case Alignment::SAME_UNALIGNED:
        cast_dbias_kernel<nvec, false, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            input, output, dbias_workspace, scale, amax, scale_inv, params, N,
            num_aligned_elements);
        break;
      case Alignment::DIFFERENT: {
        // If the pointers are aligned differently we cannot vectorize
        cast_dbias_kernel<1, true, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            input, output, dbias_workspace, scale, amax, scale_inv, params, N, N);
        break;
      }
    }
  }
...
template <int nvec, typename Param, fp32 (*OP)(fp32, const Param &), typename InputType,
          typename OutputType>
void VectorizedCastDBiasDGeluKernelLauncher(const InputType *grad, const InputType *input,
                                            OutputType *output, fp32 *dbias_workspace,
                                            const fp32 *scale, fp32 *amax, fp32 *scale_inv,
                                            const size_t N, const Param params,
                                            cudaStream_t stream) {
  if (N != 0) {
    auto align = CheckAlignment(N, nvec, grad, input, output, dbias_workspace);
    ...
    switch (align) {
      case Alignment::SAME_ALIGNED:
        cast_dbias_dgelu_kernel<nvec, true, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            grad, input, output, dbias_workspace, scale, amax, scale_inv, params, N,
            num_aligned_elements);
        break;
      case Alignment::SAME_UNALIGNED:
        cast_dbias_dgelu_kernel<nvec, false, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            grad, input, output, dbias_workspace, scale, amax, scale_inv, params, N,
            num_aligned_elements);
        break;
      case Alignment::DIFFERENT: {
        cast_dbias_dgelu_kernel<1, true, fp32, Param, OP><<<num_blocks, threads, 0, stream>>>(
            grad, input, output, dbias_workspace, scale, amax, scale_inv, params, N, N);
        break;
      }
    }
  }
