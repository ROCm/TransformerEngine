...
// Forward attention request. is_group_mode selects between batch (BSHD/SBHD)
// and group (THD/varlen) layouts; group-mode ignores batch strides and bias
// fields, batch-mode ignores cu_seqlen and max_tokens fields.
struct CkAttnFwdArgs {
...
// Backward attention request. Same is_group_mode convention.
struct CkAttnBwdArgs {
  DType dtype = DType::kNumTypes;
  bool is_group_mode = false;
  ...
  uint64_t b = 0, h = 0, hg = 0, s_q = 0, s_kv = 0, d_qk = 0, d_v = 0;
  uint64_t bias_b = 0, bias_h = 0;        // batch mode
  uint64_t max_tokens_q = 0, max_tokens_kv = 0;  // group mode
  ...
  // O / LSE / dO (forward outputs feeding the backward)
