...
void set_aiter_asm_dir() {
  ...
  std::call_once(aiter_asm_dir_once, []() {
    ...
    const char* aiter_asm_dir = std::getenv("AITER_ASM_DIR"); // check if user has set AITER_ASM_DIR, if yes, skip auto setting and log the value if NVTE_LOG_CK_CONFIG is set
    if (aiter_asm_dir) {
      if (log_ck_config) {
