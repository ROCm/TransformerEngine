...
# Test transA=False, transB=True case (NT layout)
# This is common in transformers for computing QK^T
M, N, K = 128, 128, 256
transa, transb = False, True

print("=" * 60)
print(f"Testing transA={transa}, transB={transb} (NT layout)")
print("Common use case: Q @ K^T in attention")
print("=" * 60)

torch.manual_seed(42)

# With transpose, input shapes are different
if transa:
    a_shape = (K, M)  # A is K x M, will be transposed to M x K
else:
    a_shape = (M, K)

if transb:
    b_shape = (N, K)  # B is N x K, will be transposed to K x N
else:
    b_shape = (K, N)
...
print(f"A shape: {a_shape} {'(will be transposed)' if transa else ''}")
print(f"B shape: {b_shape} {'(will be transposed)' if transb else ''}")
...
# For transA=False, transB=True: C = A @ B^T
ref_fp32 = torch.matmul(a_fp32.T if transa else a_fp32,
                        b_fp32.T if transb else b_fp32)
